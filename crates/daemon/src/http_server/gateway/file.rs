...
pub async fn handle(
    ...
    if is_html || is_markdown {
        let (final_content, final_mime_type) = if is_markdown {
            ...
            let html = super::directory::markdown_to_html(&content_str);
            let rewritten =
                super::directory::rewrite_relative_urls(&html, absolute_path, bucket_id);
            (rewritten.into_bytes(), "text/html; charset=utf-8")
        } else {
            let content_str = String::from_utf8_lossy(&file_data);
            let rewritten =
                super::directory::rewrite_relative_urls(&content_str, absolute_path, bucket_id);
            (rewritten.into_bytes(), "text/html; charset=utf-8")
        };
