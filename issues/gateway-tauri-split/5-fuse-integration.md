...
**Modify:** `crates/app/src/daemon/database/` - Add mount queries

### 5. Add REST API endpoints

**Create:** `crates/app/src/daemon/http_server/api/v0/mounts/`
...
**Create:** `crates/app/src/ops/mount.rs`
...
**Modify:** `crates/app/src/daemon/state.rs` - Add MountManager
**Modify:** `crates/app/src/daemon/process/mod.rs` - Auto-mount on startup, unmount on shutdown
**Modify:** `crates/app/Cargo.toml` - Add `jax-fuse` dependency behind feature flag
...
| Create | `crates/app/src/daemon/database/mount_queries.rs` | SQLite queries |
| Create | `crates/app/src/daemon/http_server/api/v0/mounts/` | REST endpoints |
| Create | `crates/app/src/ops/mount.rs` | CLI commands |
| Create | Migration SQL | fuse_mounts table |
| Modify | `Cargo.toml` | Add workspace member |
| Modify | `crates/app/Cargo.toml` | Add jax-fuse dep (feature-gated) |
| Modify | `crates/app/src/daemon/state.rs` | Add MountManager |
| Modify | `crates/app/src/daemon/process/mod.rs` | Auto-mount, shutdown |
| Modify | `crates/app/src/main.rs` | Add mount subcommand |
