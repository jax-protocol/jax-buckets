...
**CLI changes:**
- `jax init --port <port>` (replaces `--app-port` and `--gateway-port`)
- `jax daemon --port <port>` (optional override, defaults from config)
...
**Config changes:**
- `AppConfig.port` (single field, replaces `app_port` + `gateway_port`)
- `ServiceConfig.port` (single field)

**Dev environment:**
- `nodes.toml` now has single `port` field per node
- Ports: full=8081, app=8082, gw=8083 (8080 reserved for local development)

## Verification

```bash
# Start daemon
jax daemon --port 8080
...
curl http://localhost:8080/buckets  # 404
```
